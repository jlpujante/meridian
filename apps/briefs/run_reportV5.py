...
from src.utils import is_debug_mode, get_env_var


if is_debug_mode():
    print("**** DEBUG MODE ****")
    print(f"MERIDIAN_SECRET_KEY => {get_env_var('MERIDIAN_SECRET_KEY')}")
    print(f"SCRAPER_URL => {get_env_var('SCRAPER_URL')}")
    print("")


scraper_url = get_env_var("SCRAPER_URL") or None
# events_date = "2025-04-01"
events_date = None
# sources, events = get_events(date="2025-04-01", host="http://localhost:8787")
sources, events = get_events(date=events_date, host=scraper_url)
...
def optimize_clusters(embeddings, umap_params, hdbscan_params):
    ...
    for n_neighbors in umap_params["n_neighbors"]:
        ...
        reducer = umap.UMAP(
            ...
            # <======================= era 10, pero da el error "Cannot use scipy.linalg.eigh for sparse A with k >= N. Use scipy.linalg.eigh(A.toarray()) or reduce k."
            # n_components=8 => funciona
            # n_components=6 => funciona
...
umap_embeddings = umap.UMAP(
    ...
    n_neighbors=15,
    n_components=N_COMPONENTS,  # bumped up from 5 <=== era 10, pero da el error
    min_dist=0.0,
    metric="cosine",
...
print("len(cleaned_clusters) => " + str(len(cleaned_clusters)))
...
#         f"{scraper_url}/last-report",
#         headers={
#             "Authorization": f"Bearer {get_env_var('MERIDIAN_SECRET_KEY')}"
#         }
#     )
